...
namespace jeanbaptiste::core
{
    ...
    public:
        ...
        void executeRecursion(Complex* data, unsigned groupNodeIdx, unsigned totalSampleCnt) const
        {
            ...
            Complex twiddleMultiplier(
                ...
                static_cast<typename Complex::value_type>(DirectionFactor * basic::sine<typename Complex::value_type>(2, SampleCnt::value));
            ...
            for (unsigned currentGroupIdx = groupNodeIdx, groupIdxEnd = (groupNodeIdx + quaternaryNodeDistance); currentGroupIdx < groupIdxEnd; ++currentGroupIdx)
            {
                ...
                Complex::value_type temp = 1.5 - 0.5 * (twiddleFactor.real() * twiddleFactor.real() + twiddleFactor.imag() * twiddleFactor.imag());
                ...
                while (segmentIdx < totalSampleCnt)
                {
                    for (unsigned tupleIdx = segmentIdx; tupleIdx < totalSampleCnt; tupleIdx += lShapedNodeDistance)
                    {
                        ...
                        Complex temp1 = data[idxNode2] * wn4;
                        Complex temp2 = data[idxNode3] * w3n4;
                        // Attention: in order to make bit reversal more comfortable slightly customize the formula.
                        // original: temp3 = temp1 - temp2;
                        Complex temp3 = temp2 - temp1;
                    
                        temp1 += temp2;
                        temp2 = temp3 * Complex(0, DirectionFactor);
    ...
    */
    ...
    class SplitRadixDIF
    {
        SplitRadixDIF<SampleCnt::value / 2, DirectionFactor, T, Complex> recursionLevel_;
    ...
    public:
        ...
        void executeRecursion(Complex<T>* data, unsigned groupNodeIdx, unsigned totalSampleCnt)
        {
            ...
            Complex<T> twiddleMultiplier(static_cast<T>(-2.0 * Sine<1, SampleCnt::value, T>::value() * Sine<1, SampleCnt::value, T>::value()),
                                         static_cast<T>(DirectionFactor * Sine<2, SampleCnt::value, T>::value()));
            // Create transform factor.
